...
export default function Dashboard() {
  ...
  /* ===== API ENDPOINT MAP (FIXED: /api added) ===== */
  const getEndpoint = (section) => {
    const map = {
      Career: "/api/careers",
      Roadmaps: "/api/roadmaps",
      Videos: "/api/videos",
      Documentations: "/api/docs",
      "Best Links": "/api/links",
      "Best News": "/api/news",
    };
    return map[section];
  };

  /* ===== READ URL PARAMS (query + section) ===== */
  useEffect(() => {
    const params = new URLSearchParams(location.search);
    ...
    if (section === "auto" && query) {
      const q = query.toLowerCase();
      section =
        q.includes("ai") || q.includes("ml") || q.includes("devops")
          ? "Roadmaps"
          : "Career";
    }
  ...
  useEffect(() => {
    ...
    setLoading(true); // ✅ show loader immediately when section/search changes
  ...
  const ActiveSectionComponent = SECTION_COMPONENTS[activeSection] || null;
    ...
    <div className="flex h-screen bg-slate-900">
      ...
      <main className="flex-1 overflow-y-auto p-6 pb-20 md:pb-6">
        ...
        {loading ? (
          ...
          ActiveSectionComponent && <ActiveSectionComponent data={data} />
        )}
      </main>
